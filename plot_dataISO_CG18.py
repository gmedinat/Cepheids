...
for f in oc_list :
	...
	if True and os.path.exists('Clusters/%s/%s_bestIsochrone_CG18.csv'%( f , f )):
		...
		dfiso = pd.read_csv( 'Clusters/%s/%s_bestIsochrone_CG18.csv'%( f , f ) )	
		...
		df_0 = pd.read_csv( 'Clusters/%s/%s_starsFromCG18_matchGaiaDR2_wAbsMag_bestIso_CG18.csv'%( f , f ) )	
		...
		print ra_cep, dec_cep
		print G_cep, BP_cep - RP_cep
		...
		fig, ax = plt.subplots(2, figsize=(12.5, 7), sharex=True) # largo, ancho

		fig.suptitle('%s\nSearch rad: %.2f deg       Z: %.2f    logA: %.2f    EBV: %.2f    dist: %i'%(f,search_rad,Z,logAge,EBV,dist), fontsize=16)
		...
		if AbsAp == 'Abs':
			...
			plt.plot(BP-RP, G, 'o', color=color_OC, label='OC stars', markersize=5)
			plt.plot(BP_iso - RP_iso, G_iso, '-', color=color_iso, label='Isochrone', linewidth = 4, linestyle = '-.')
			plt.plot(BP_cep-RP_cep, G_cep, '*', color=color_cep, label='S Nor', markersize=12)
			
			#plt.axis([min(BP - RP)-1, max(BP - RP) + 2, max(G) + 1.3, min(G) - 2])
			plt.axis([min( min(BP - RP), min(BP_cep-RP_cep) )-1, max( max(BP - RP), max(BP_cep-RP_cep) ) + 2, max( max(G), max(G_cep) ) + 1.3, min( min(G), min(G_cep) ) - 2])
		...
		if save == True:
			if AbsAp == 'Abs':
				...
				#plt.savefig('./Clusters/%s/%s_radecCMDwISO_CG18.png'%(f,f))
				plt.savefig('./Clusters_afterPreselection/%s_radecCMDwISO_CG18.png'%(f))
