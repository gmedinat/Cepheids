...
df['vr_diff']       = df['RV'] - df['RV_gaiaravevsmelnik_MELNIK']
	...
	if (cep_par == 0 and ecep_par == 0) or (oc_par == 0 and eoc_par == 0): # as==0
		...
		if (cep_rv == 0 and ecep_rv == 0) or (oc_rv == 0 and eoc_rv == 0):	# bs==0	 
			...
			if (cep_pmdec == 0 and ecep_pmdec == 0) or (oc_pmdec == 0 and eoc_pmdec == 0): # ds == 0
				...
				M1 = subdf['e_RV']**2
				# covariance matrix cepheid
				M2 = subdf['e_RV_gaiaravevsmelnik_MELNIK']**2
				...
				# covariance matrix cepheid
				M2 = np.matrix([[subdf['e_RV_gaiaravevsmelnik_MELNIK']**2, cov_rv_pmdec],
			...
			# covariance matrix cepheid
			M2 = np.matrix([[subdf['e_RV_gaiaravevsmelnik_MELNIK']**2, cov_rv_pmra],
			...
			# covariance matrix cepheid
			M2 = np.matrix([[subdf['e_RV_gaiaravevsmelnik_MELNIK']**2, cov_rv_pmra, cov_rv_pmdec],
...
df.to_csv( './'+catalog_name+'_wProbs', index=False )
