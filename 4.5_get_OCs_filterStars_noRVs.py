...
while i < largo:
	...
	if (not Cluster_Name in str(lista_clusters)):
		...
		print '\tfrom ', catalogFile_name, '   to ', file_name.replace('.vot','_noRVs.csv')
		...
		if A == True and B == True and C == True: 
			...
			df3.to_csv( file_name.replace('.vot','_noRVs.csv'), index=False )
			...
			df2.to_csv( file_name.replace('.vot','_noRVs.csv'), index=False )
			...
			df1 = df_OC.loc[(df_OC['pmra'] >= pmRA_cluster-sigma_num*epmRA_cluster) & (df_OC['pmra'] <= pmRA_cluster+sigma_num*epmRA_cluster)]	
			df1.to_csv( file_name.replace('.vot','_noRVs.csv'), index=False )
