...
highprob_catalog_name = 'MatchAndWithin_D02+K13_Gaia_2deg_5r1_12-04-19_dupRemoved_wPrior_RVMelnikOnly_ErrsTweak_covMatTweak_wProbs.csv'
...
catalog_name = './Clusters/%s/%s_starsFromCG18_matchGaiaDR2.csv'%(cluster_name,cluster_name)
df_0 = pd.read_csv(catalog_name)
#df = df_0.loc[df_0['PMemb'] >= 0.5]
df = df_0.loc[df_0['PMemb'] >= 0.5].copy()
...
if float(FeH_guess_ini) == 0 or np.isnan(FeH_guess_ini): # no info is available. Assume solar met. as initial guess
	Z_guess_ini = 0.0152
else:
...
if (Z_guess - 0.003) <= 0.006: # given by the limits in the isochrones I downloaded
	initial_Z = 0.006
	final_Z   = Z_guess + 0.003 	
elif (Z_guess + 0.003) >=  0.029: # given by the limits in the isochrones I downloaded
	initial_Z = Z_guess - 0.003
	final_Z   = 0.029
else:
	initial_Z = Z_guess - 0.003
	final_Z   = Z_guess + 0.003	
...
min_redchi2 = 1000000000

initial_age = 710 # logAge x 100
final_age = 800 # logAge x 100
...
if age_guess == 0:
	...
	initial_age = age_guess - 30  	
	final_age = age_guess + 30 

print '\n'
print age_guess_ini
print age_guess
print initial_age, final_age
#print round(age_guess_ini)
#print round(age_guess)
#print round(initial_age), round(final_age)
#sys.exit(0)	
...
#print current_initial_age, current_final_age
#sys.exit(0)
...
while current_initial_age < final_age: # this is to open the files. Each file has different metallicities, different ages in the range (current_initial_age,current_final_age)
	...
	while index_met < len_diff_met: # con esto se recorre el set completo de isocronas
		...
		while index_age < len_diff_ages: # con esto se recorre el set completo de isocronas con metalicidad constante
			...
			df_set_age_0 = df_set_met.loc[df_set_met['log_age_yr'] == diff_ages[index_age]]

			largo_set_age = len(df_set_age_0['Zini']) # numero de puntos con edad especifica
			...
			if verbose == False: # print at least some information
				...
				#	print '\n'
				#	print '------------------------------------------------------------------------------------------------------------------------------'
				print '\tZ = %s \tlogAge = %s'%(df_set_age_0['Zini'].iloc[0], df_set_age_0['log_age_yr'].iloc[0]) 
			...
			while j < largo_grid_ebv: # con esto se recorre el set de isocronas con metalicidad, edad y color excess constante
				...
				if verbose == True:
					...
					print cluster_name, '\tZ_range: [%.4f, %.4f] \t logAge_range: [%.2f, %.2f] \t E(B-V)_range: [%.2f, %.2f] \t dist_range: [%i, %i]'%( initial_Z, final_Z, initial_age, final_age, EB_V_range_min, EB_V_range_max, dist_range_min, dist_range_max) 
				...
				while k < largo_grid_dist: # con esto se recorre el set de isocronas con metalicidad, edad, color excess y distancia constante
					...
					#df_set_age = df_set_age_0
					#print len(df_set_age_0), max(g_abs), len(df_set_age)
					...
					if avg_redchi2 < min_redchi2:
						...
						#print df_set_age['G']
						#print g_abs
						#print df_set_age['G_BP'] - df_set_age['G_RP']
						#print bp_abs - rp_abs
						#print avg_redchi2_min
...
df.to_csv( catalog_name.replace('.csv','_wAbsMag_bestIso_CG18.csv'), index=False )
...
bestisoc_name = './Clusters/%s/%s_bestIsochrone_CG18.csv'%(cluster_name,cluster_name)
