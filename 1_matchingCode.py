...
while i < len_ocs:
	...
	angular_rad = df_ocs.iloc[i][angrad_oc_name]
	...
	while j < len_cep:
		...
		#dist = (((ra_oc-ra_cep)**2.+(dec_oc-dec_cep)**2.)**0.5)
	
		#dist = (((df_ocs.iloc[i][ra_oc_name]-df_cep.iloc[j][ra_cep_name])**2.+(df_ocs.iloc[i][dec_oc_name]-df_cep.iloc[j][dec_cep_name])**2.)**0.5)
		...
		#if dist%360 <= 2:
		#	print df_cep.iloc[j]['designation'], '\t\t', dist, '\t\t', dist%360
		#	print '\t\t\t', sep, sep.degree
